package com.tang.app_common.interceptor;
...
public class DefaultFormatPrinter implements FormatPrinter {
    ...
    public void printJsonResponse(long chainMs, boolean isSuccessful, int code, @NonNull String headers, @Nullable MediaType contentType,
        ...
        bodyString = HttpLoggingInterceptor.isJson(contentType) ? CharacterHandler.jsonFormat(bodyString)
                : HttpLoggingInterceptor.isXml(contentType) ? CharacterHandler.xmlFormat(bodyString) : bodyString;
        ...
        LogUtil.d(tag, RESPONSE_UP_LINE);
    ...
    private static void logLines(String tag, String[] lines, boolean withLineSize) {
        for (String line : lines) {
            ...
            for (int i = 0; i <= lineLength / MAX_LONG_SIZE; i++) {
                ...
                LogUtil.d(resolveTag(tag), DEFAULT_LINE + line.substring(start, end));
            }
        }
    }
